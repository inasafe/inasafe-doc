...
.. note:: If you don't see these colours, you might need to turn off the data layer above the *Estimated building affected* layer.
...
-  The red buildings are situated in water greater than one metre, the
   ...
   metre, while the green buildings are not affected because they are in
   water that is less than the threshold depth of one metre.
...
-  **Hazard Category**: divides the results into several categories
   ...
   threshold), and number of dry building (buildings that are not
...
-  **Detailed building type report:** statistical breakdown of the building
   types affected by the hazard event. When you choose to use an aggregation
...
Bt reducing the flood threshold value from 1.0 to 0.8, more buildings will be
...
- **Notes:** provides details about the input data and any limitations or
  ...
  shows the total number of people in the analysis area and the source of
  minimum needs.
...
on **Head of Indonesia National Disaster Management Authority, BNPB,
